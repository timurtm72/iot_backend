...
@Table(name = "relay") // Задает имя таблицы в базе данных как "relay" (исправлено с relay_device для соответствия SQLDelete)
@SQLDelete(sql = "UPDATE relay SET is_removed = true WHERE id = ?") // Определяет SQL-запрос для мягкого удаления
...
public class Relay {
    ...
    /**
     * Коллекция значений состояния реле (включено/выключено) с временными метками.
     * Использует ленивую загрузку для оптимизации производительности.
     * Хранится в отдельной таблице "relay_values".
     */
    @ElementCollection(fetch = FetchType.LAZY) // Определяет коллекцию встраиваемых объектов с ленивой загрузкой
    @CollectionTable(
        name = "relay_values", // Имя таблицы для хранения коллекции
        joinColumns = @JoinColumn(name = "relay_id") // Имя колонки для связи с таблицей relay (исправлено с relay_device_id)
    )
    @OrderBy("timestamp DESC") // Сортировка значений по временной метке в порядке убывания (последние первыми)
    private List<BitData> relayValues = new ArrayList<>(); // Поле для хранения списка значений реле, инициализировано пустым списком
}